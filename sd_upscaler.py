...
controlnet_path = os.getenv("CONTROLNET_MODEL_PATH")
...
text_inversion1_path = os.getenv("TEXT_INVERSION_PATH_1")
text_inversion2_path = os.getenv("TEXT_INVERSION_PATH_2")
upscale_model = os.getenv("UPSCALE_MODEL")


if not all([controlnet_path, sd_model_path, vae_path, lora_weights1_path, lora_weights2_path, text_inversion1_path, text_inversion2_path]):
...
class LazyLoadPipeline:
    ...
    def setup_pipeline(self):
        ...
        pipe.vae = vae
        pipe.load_textual_inversion(text_inversion1_path)
        pipe.load_textual_inversion(text_inversion2_path)
...
class LazyRealESRGAN:
    ...
    def load_model(self):
        if self.model is None:
            ...
            self.model.load_weights(f'/gorgon/ia/ComfyUI/models/upscale_models/RealESRGAN_x{self.scale}.pth', download=False)
...
@timer_func
def process_image(input_image, resolution, num_inference_steps, strength, hdr, guidance_scale, controlnet_strength, scheduler_name):
...
def main():
    ...
    if args.output_directory:
        ...
        # Create directory with 'upscaled_' prefix if only one image is provided
        base_path, _ = os.path.splitext(args.input_image)
        output_dir = f"{base_path}_upscaled"
    ...
    _, final_result = process_image(input_image, resolution, num_inference_steps, strength, hdr, guidance_scale, controlnet_strength, scheduler_name)
